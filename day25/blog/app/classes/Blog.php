...
//            return $message;
    ...
    public function updateBlogInfoById($data,$id){
        ...
        if ($fileName){
                ...
                if(file_exists($imageUrl)) {
                    ...
                    if($fileSize > 5000000) {
                        ...
                        if($fileType != 'jpg' && $fileType != 'png') {
                            ...
                            move_uploaded_file($_FILES['blog_image']['tmp_name'], $imageUrl);
                            $sql = "UPDATE blogs SET category_ID = '$data[category_id]', blog_title = '$data[blog_title]', short_description = '$data[short_description]', long_description = '$data[long_description]',blog_image = '$imageUrl', publication_status = '$data[publication_status]' WHERE id = '$id'";
                            if (mysqli_query(Database::dbConnection(), $sql)) {
                                $message = 'Blog information save successfully';
                                return $message;
                            } else {
            ...
            $sql = "UPDATE blogs SET category_ID = '$data[category_id]', blog_title = '$data[blog_title]', short_description = '$data[short_description]', long_description = '$data[long_description]', publication_status = '$data[publication_status]' WHERE id = '$id'";
            if(mysqli_query(Database::dbConnection(),$sql)){
